...
const pilotSourceStatus = (sourceCollectionStatus.items || []).find(item => item.source_key === "pilot_sources") || {};
const pilotSourceDiagnostics = Array.isArray(pilotSourceStatus.diagnostics) ? pilotSourceStatus.diagnostics : [];
...
const statusRawPilotRows = Number(pilotSourceStatus.rows_collected || 0);
const statusNormalizedPilotRows = Number(pilotSourceStatus.rows_normalized || 0);
const rawPilotRows = collectorRawPilotRows || statusRawPilotRows;
const normalizedPilotRows = Number(collectorPilot.pilot_rows_normalized || 0) || statusNormalizedPilotRows;
const sourceRowsWith = key => pilotSourceDiagnostics.reduce((sum, diag) => sum + Number(diag[key] || 0), 0);
const persistedEventsLoaded = Number(pilotageEnrichment.persisted_events_loaded || 0);
const sourceConfigured = Boolean(String(process.env.PILOT_SOURCE_URLS || "").trim()) || Boolean(pilotSourceStatus?.env_presence?.PILOT_SOURCE_URLS?.present) || Number(collectorPilot.pilot_sources_attempted || 0) > 0;
const sourceEnabled = sourceConfigured || rawPilotRows > 0;
const fetchAttempted = Number(collectorPilot.pilot_sources_attempted || 0) > 0 || Boolean(pilotSourceStatus.collector_attempted);
...
if (sourceEnabled && rawPilotRows === 0) warnings.push("pilot_sources enabled but raw_pilot_rows=0");
if (rawPilotRows > 0 && insertedEvents === 0) warnings.push("raw pilot rows exist but inserted_pilot_schedule_events=0");
...
console.log(`pilot_source_rows_collected=${rawPilotRows}`);
console.log(`raw_pilot_rows=${rawPilotRows}`);
...
console.log(`normalized_pilot_rows=${normalizedPilotRows}`);
console.log(`pilot_rows_with_vessel_name=${Number(collectorPilot.pilot_rows_with_vessel_name || sourceRowsWith("pilot_rows_with_vessel_name") || 0)}`);
console.log(`pilot_rows_with_call_sign=${Number(collectorPilot.pilot_rows_with_call_sign || sourceRowsWith("pilot_rows_with_call_sign") || 0)}`);
console.log(`pilot_rows_with_port=${Number(collectorPilot.pilot_rows_with_port || sourceRowsWith("pilot_rows_with_port") || 0)}`);
console.log(`pilot_rows_with_pilot_date=${Number(collectorPilot.pilot_rows_with_pilot_date || sourceRowsWith("pilot_rows_with_pilot_date") || 0)}`);
console.log(`pilot_rows_with_pilot_time=${Number(collectorPilot.pilot_rows_with_pilot_time || sourceRowsWith("pilot_rows_with_pilot_time") || 0)}`);
console.log(`pilot_rows_with_pilot_station=${Number(collectorPilot.pilot_rows_with_pilot_station || sourceRowsWith("pilot_rows_with_pilot_station") || 0)}`);
console.log(`pilot_rows_with_pilot_direction=${Number(collectorPilot.pilot_rows_with_pilot_direction || sourceRowsWith("pilot_rows_with_pilot_direction") || 0)}`);
...
console.log(`unmatched_pilot_rows=${Number(pilotageEnrichment.unmatched_pilot_rows || Math.max(0, rawPilotRows + persistedEventsLoaded - pageSignalSummary.has_pilotage_count) || 0)}`);
console.log(`match_blockers=${JSON.stringify(pilotageEnrichment.match_blockers || pilotageEnrichment.sample_needs_review || [])}`);
